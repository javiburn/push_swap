...
/*   Updated: 2023/05/04 14:39:04 by jsarabia         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_chunk	algorithm(t_chunk chunk)
{
	...
	if (chunk.argcs < 50)
	{
		...
		return (movements_undefined_b(chunk, chunk.argcs - 1));
	}
	if (chunk.argcs < 500)
	{
		chunk = movements_undefined(chunk, 10);
		return (movements_undefined_b(chunk, chunk.argcs - 1));
	}
	else
	{
		chunk = movements_undefined(chunk, 45);
		return (movements_undefined_b(chunk, chunk.argcs - 1));
	}
	return (chunk);
}
